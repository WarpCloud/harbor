...
const (
	...
	defaultImageTagFilterPath          = "/etc/core/image_tag.yaml"
)
...
var (
	...
	imageTagFilterConfig *comcfg.ImageTagFilterConfig
)


func InitImageTagFilterConfig() error {

	log.Info("init load config /etc/core/image_tag.yaml")

	if utils.FileExists(defaultImageTagFilterPath) {

		log.Info("found config /etc/core/image_tag.yaml, begin reading... ")

		data, err := ioutil.ReadFile(defaultImageTagFilterPath)
		if err != nil {
			return err
		}
		if err = yaml.Unmarshal(data, &imageTagFilterConfig); err != nil {
			return err
		}

		log.Info("load config /etc/core/image_tag.yaml suc...")
	} else {
		log.Info("not found config /etc/core/image_tag.yaml, skip load the config... ")
	}
...
func ReloadImageTagFilterConfig() {

	log.Info("reload config /etc/core/image_tag.yaml")
	err := InitImageTagFilterConfig()
	if err != nil {
		log.Warningf("reload ImageTagFilter config failed ... ")
	}
...
// GetImageTagFilterConfig return the current config
func GetImageTagFilterConfig() *comcfg.ImageTagFilterConfig {
	return imageTagFilterConfig
}
