...
func main() {
	...
	if  err := config.InitImageTagFilterConfig(); err != nil {
		log.Fatalf("Failed to initialize config  /etc/core/image_tag.yaml.  with error: %s", err.Error())
	}
	...
	// transwarp add image tag filter
	beego.InsertFilter("/v2/*", beego.BeforeRouter, filter.TagCheckFilter)
